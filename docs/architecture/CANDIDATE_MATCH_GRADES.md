...
- **本次交付：** 新增 migration 019、独立评估服务、MCP 保存接口、准备资料读取、
  ...
  已发布至生产，详见文末证据；候选决定、申请状态、已投递材料及定时任务不在本包范围内。
...
本次承接用户“请先测试 Chrome 连接，成功后继续上线”的授权。Chrome 连接、
已登录 AWS 页面读取和 Lightsail 浏览器 SSH 均成功；原访问阻塞解除。
发布使候选评级存储及 Jobs 展示进入生产，为真实 JD 评估、来源版本及跨会话
读取验收提供入口。长期目标仍为可追溯的岗位选择依据；本次不创建真实评级。

- 实例：既有 `paw-mvp`；旧镜像 `application-resume-ui-20260911-r1`，
  新镜像 `candidate-grades-20260911-r1`，于 `2026-09-11T10:32:33Z`
  （悉尼 20:32）健康切换。运行镜像 ID 为
  `sha256:23b6a6220d718fcb40c91f9f301c21e6d5283752d4b6a84fb7f4e7fd47a3cec9`。
- 固定源码继续使用上节 `6fef9b7` 及归档 SHA-256；服务器传输后逐一校验通过。
  服务器源码为 `/opt/paw-candidate-grades-20260911-r1`。沿用既有私有 S3
  临时传输位置、全部 public-access blocks 和 AES256；上传对象的精确版本
  已删除，本地临时签名 URL 文件也已删除。未更改防火墙、身份或密钥。
- 复用此前同一运行输入的 444 项测试、两套类型检查、构建与桌面/390px
  合成页面证据。云端镜像构建通过；临时 build/cutover 脚本 `bash -n` 通过。
- 一致备份 `workspace-20260911T103154Z.db` 完整性通过。隔离副本演练通过
  019 迁移、候选重复启动及旧镜像读取升级副本：48 张表、2,102 行；
  三次启动健康且数据完整。迁移验证保留原有 46 张业务表及原迁移记录，
  唯一新增表为 `candidate_match_assessments`，为空。
- 切换前再次一致备份 `workspace-20260911T103225Z.db`。停止应用并 checkpoint
  后保留完整数据库目录副本，迁移后再复制验证；正式迁移同样通过原表逐行摘要、
  schema、外键、完整性及迁移历史保留检查。没有用旧数据库覆盖实时数据。
  前后副本及发布日志位于 `/srv/paw/deployments/candidate-grades-20260911-r1-*`。
- 公网 `npm.cmd run web:check -- --origin https://workspace.ai-radar-lab.com --writes off`
  五项全部通过。实际运行容器 MCP 发现 31 个工具（包括新保存命令），
  `workspace_ping` 返回原 Workspace 与可用数据库；10 个候选都有评级摘要。
  Google 候选的已保存 JD 返回 `UNASSESSED`，另一缺失 JD 的候选返回
  `MISSING_JD` 且无评级。基础简历版本 8，所测准备上下文 25,516 bytes。
  只读探针未调用保存接口，外部匹配为 unset/off，general Web writes 为 false。

生产页面刷新后曾转到 Google 登录选择页；用户登录后完成下述最终页面检查。
真实评级保存、评估质量、用户纠正、资料更新后的失效及刷新后的 ChatGPT
连接器跨会话使用仍需独立验收，不能以本次只读探针或既有合成测试替代。
定时任务配置和实际无人值守运行验收保持独立。

### Authenticated Jobs acceptance — September 11

用户确认已登录后，使用同一 Chrome 会话检查生产页面：

- Jobs 列表正常显示 10 个候选。Google 的已保存 JD 显示“待评估 / 尚未保存
  ChatGPT 的岗位评估”，其余 9 个候选显示“JD 待补充 / 完整 JD 待补充”。
- 点击 Google 的职位标题进入详情；岗位匹配区、已保存 JD 和职位简历入口正常显示，
  评级状态与列表及发布时 MCP 的 `UNASSESSED` 读取一致。
- Balance Recruitment 缺失 JD 的详情显示补充说明及原有 JD 编辑控件；
  匹配状态与列表及 MCP 的 `MISSING_JD` 读取一致，没有显示虚构等级。
- Google 详情截图检查通过，1546px 实际桌面视口的内容宽度同为 1546px，
  无横向溢出。复用同一代码已有的 390px 合成布局证据，本轮未重复窄屏测试。
- 本轮只导航和读取页面，没有保存 JD、改变候选决定、创建简历或写入评级。
