...
**状态：已纳入待办，尚未实现。** 用户希望沿用自己一直使用的 GPT 岗位评估方式，
在职位列表中直接比较申请优先级。本次只记录需求和验收条件；下一步设计保存/读取
契约，再实现列表和详情展示。近期收益待实现后验证：减少重复评估与翻找聊天；
...
- [ ] 列表展示评级和一句主要理由；无 JD、未评估、评估已过期分别明确呈现。
- [ ] 详情展示逐项岗位要求、对应经历证据、优势、差距与待确认事项。
- [ ] 保存评级标准版本、JD 来源/版本、简历版本、所用经历来源和评估时间；事实与推断分开。
- [ ] JD 或所用简历/资料变化后标记待重新评估，保留历史结果及用户纠正依据。
- [ ] 保存遵守身份、归属、版本与幂等规则；重复提交不产生重复评估记录。
