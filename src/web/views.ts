...
function resumeTargetLinks(service:WorkspaceService,type:"CANDIDATE"|"APPLICATION",id:string){
  const copies=service.resumeService.listVariants().filter(v=>type==="CANDIDATE"?v.candidateId===id:v.projectId===id);
  return `<div class="resume-target-links"><strong>为这个职位准备的简历</strong>${copies.map(v=>`<a href="${rootPath}/resume/variants/${v.id}">${e(v.name)} · 版本 ${v.recordVersion} ↗</a>`).join("")}<a class="button secondary" href="${rootPath}/resume?${type==="CANDIDATE"?"candidateId":"projectId"}=${id}#resume-versions">＋ 创建职位简历</a><small class="muted">独立编辑版本；实际投递记录另行确认。</small></div>`;
}
