...
export function applicationListView(service: WorkspaceService, query: Record<string, string | number>, zone: string, gmailEnabled = false): string {
  ...
  query = { ...query, status: query.status ?? "ONGOING" };
...
export function candidateListView(service: WorkspaceService, query: Record<string, string | number>, zone: string, matchingEnabled=false): string {
  ...
  const rows = page.items.map((candidate: JobCandidateRecord) => {
    ...
    const fitLabel = !fit ? "待匹配" : fit.library_hash !== libraryHash ? "资料已更新 · 待重评" : fit.score === null ? "待核对" : `${fit.score}% 匹配`;
    const source = safeExternalUrl(candidate.sourceUrl);
    return `<article class="candidate-row"><div class="grow"><p class="overline">${e(candidate.company)}${candidate.location ? ` · ${e(candidate.location)}` : ""}</p><h2><a href="${candidateLink(candidate.id)}">${e(candidate.role)}</a></h2><p class="muted">${e(candidate.title)}</p>${candidate.fitReason ? `<p class="advisory">${e(candidate.fitReason)}</p>` : ""}</div><div class="row-status">${chip(candidate.decision)}<strong>${e(fitLabel)}</strong>${candidate.linkedProjectId ? `<a class="text-link" href="${appLink(candidate.linkedProjectId)}">已关联申请 ↗</a>` : `<span class="muted">未关联</span>`}${source ? `<a class="text-link" href="${e(source)}" target="_blank" rel="noopener noreferrer">职位来源 ↗</a>` : ""}</div></article>`;
  }).join("");
  return document("职位", `${heading("JOBS / 职位", "候选职位，逐条决定", "收藏、忽略或关联到实际申请；决策会跨对话保留。")}${freshness(page.asOf, zone)}${discoveryPanel(service,matchingEnabled)}<form class="filters" method="get" data-filter-form><label class="search-label">搜索公司、职位或标题<input type="search" name="q" value="${e(query.q ?? "")}" placeholder="公司、职位、标题关键词" maxlength="500"></label><label>决策<select name="decision">${["UNREVIEWED", "SAVED", "DISMISSED", "ALL"].map((x) => option(x, x === "ALL" ? "全部决策" : label(x), decision)).join("")}</select></label><label>关联<select name="linked">${option("ALL", "全部", linked)}${option("UNLINKED", "未关联", linked)}${option("LINKED", "已关联", linked)}</select></label><label>排序<select name="sort">${option("FIT_DESC", "匹配度优先", String(query.sort))}${option("UPDATED_DESC", "最近更新", String(query.sort))}</select></label><button class="button primary" type="submit">应用筛选</button></form><section class="panel"><header class="section-heading"><h2>候选职位 <span class="count">${page.totalCount}</span></h2></header><div data-page-items>${rows || empty("当前范围没有候选职位", "调整筛选，或在 ChatGPT 中记录新的候选职位。")}</div>${pagination(page, `${rootPath}/jobs`, query)}</section>`, true, "jobs");
}
