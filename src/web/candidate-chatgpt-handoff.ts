...
1. 按 candidateId 调用 workspace_get_job_candidate，启用 includeAssessmentContext，核对职位身份，读取最新已保存 JD、基础简历、资料目录和已确认 Screening Profile。按需选择相关来源并读取全文；不要仅凭对话记忆补充个人事实。
2. 如果缺少完整 JD，先告知我补充全文和来源链接。只有取得完整原文并获准保存后，才使用 workspace_record_candidate_job_description 保存，再重新读取上下文。未保存的网页编辑内容不属于分析输入。缺少已确认 Profile 或必要材料时明确说明，不猜测、不伪造确认，不生成缺乏依据的评级。
3. 沿用 Workspace 中已确认的筛选规则，区分必需项、优先项、经验事实、个人偏好和 UNKNOWN；资料不足不等于能力不足。先完成 screening，再做技能匹配分析，不以职位名称代替完整 JD 判断。
4. 用中文呈现：总体匹配等级与理由、逐项 JD 要求及原文、对应技能/经历证据与判断、主要优势、缺口、待确认问题和申请建议。引用实际资料原文与来源；无法成立的结论保持 UNKNOWN。评级与 screening 是两份独立结果。
